...
OS库是Python标准库，包含几百个函数。OS模块提供了非常丰富的方法用来处理文件和目录。
### 常用的几个函数
...
一圈下来，发现跟Linux的shell端命令差不多嘛。还行，比较好理解。
### 实例一：强大的system函数
...
以calc为例，直接在cmd窗口中输入calc后回车即可打开。

**b.打开自己安装且没有加入环境变量的程序**

  以微信为例，在cmd窗口中需要输入完整的程序路径  "A:\WeChat\WeChat.exe"。

  注意，双引号是必须的。

```python
import os
os.system("A:\WeChat\WeChat.exe")
print("打开成功！")
```
    若将网易云音乐的路径加入环境变量之中，则在cmd窗口中输入WeChat后回车即可运行。

#### 关闭命令
以关闭微信为例，这里的关闭，指的是结束任务进程。只要使用
`taskkill /f /t /im 进程名`就行了。
```python
import os
...
### 实例二：自动安装python第三方库脚本

```python
import os
libs={"numpy","requests","jieba"}
try:
	for lib in libs:
